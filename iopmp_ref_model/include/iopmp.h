...
iopmpMatchStatus_t iopmpRuleAnalyzer(iopmp_t *iopmp, iopmp_trans_req_t trans_req, uint64_t prev_iopmpaddr, uint64_t iopmpaddr, entry_cfg_t iopmpcfg, uint8_t md, int is_priority);
void errorCapture(iopmp_t *iopmp, perm_type_e trans_type, uint8_t error_type, uint16_t rrid, uint16_t entry_id, uint64_t err_addr, uint8_t *intrpt);
void generate_interrupt(iopmp_t *iopmp, uint8_t *intrpt);
