...
void generate_interrupt(iopmp_dev_t *iopmp, uint8_t *intrpt) {
    ...
    // - IOPMP doesn't implement Message-Signaled Interrupts (MSI) extension,
    //   or IOPMP implements MSI extension but MSI is disabled
    wired = iopmp->reg_file.err_cfg.ie && !iopmp->intrpt_suppress &&
            (!iopmp->imp_msi ||
             (iopmp->imp_msi && !iopmp->reg_file.err_cfg.msi_en));

    // IOPMP triggers MSI if the following conditions are true:
    // - IOPMP interrupts are enabled
    // - The interrupts are not suppressed
    // - IOPMP implements Message-Signaled Interrupts (MSI) extension and MSI is
    //   enabled
    // - There are no MSI write error
    msi = iopmp->reg_file.err_cfg.ie && !iopmp->intrpt_suppress &&
          iopmp->imp_msi && iopmp->reg_file.err_cfg.msi_en &&
          !iopmp->reg_file.err_info.msi_werr;

    // Output wired interrupt signal
    *intrpt = wired;
