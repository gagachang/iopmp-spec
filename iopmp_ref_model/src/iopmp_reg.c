 ...
 */
reg_intf_dw read_register(iopmp_t *iopmp, uint64_t offset, uint8_t num_bytes) {
 ...
 */
void rrid_stall_update(iopmp_t *iopmp, uint8_t exempt) {
 ...
 *
 */
void write_register(iopmp_t *iopmp, uint64_t offset, reg_intf_dw data, uint8_t num_bytes) {
  ...
  switch (offset) {
    ...
    case HWCFG3_OFFSET:
            ...
            if (!iopmp->reg_file.hwcfg0.enable) {
                reg_file.hwcfg3.md_entry_num = hwcfg3_temp.md_entry_num;
            }
        #endif
        #if (IOPMP_RRID_TRANSL_EN)
